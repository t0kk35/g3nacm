...
const workflow_code = 'function.entity.change_state.get'

export class FunctionChangeEntityStateGet implements IWorkflowFunction {
    code = workflow_code;
...
async function checkLease(client: PoolClient, entityId: string, entityCode: string, userName: string) {
    ...
    if (res_lease.rows.length === 0) WorkflowErrorCreators.function.userDoesNotHaveLease(origin, userName, entityId, entityCode);
}