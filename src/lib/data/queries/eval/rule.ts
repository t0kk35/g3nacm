...
import { EvalRule, EvalRuleCondition, EvalLogicalOperator, EvalComparisonOperator, RawRuleRow } from "@/lib/eval-engine/types";
...
export const queryEvalRule = defineQuery({
    path: 'eval/rule',
    permissions: [],
    params: paramsSchema,
    execute: async ({ group }: z.infer<typeof paramsSchema>, ctx: QueryContext): Promise<EvalRule[]> => {
        const conn = ctx.client ?? db.pool;
        try {
            const result = await conn.query(query_text, [ctx.userName, group]);
            return buildRules(result.rows);
        } catch (err) {
            throw new DataQueryError('Get Eval rules', err as Error);
        }
    },
});
