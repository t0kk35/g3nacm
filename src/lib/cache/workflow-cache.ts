...
export async function getCachedWorkflowConfig(entityCode: string, orgUnitCode: string) {
    const key = `workflow:${orgUnitCode}:${entityCode}`;
    
    return workflowConfigCache.get(
        key,
        async () => {
            try {
